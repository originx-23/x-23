#!/usr/bin/env python3.6
# -*- coding: utf-8 -*-

from sqlalchemy import exc

from cluster import db
from universal import x_error


class JournalTable(db.Model):
    __tablename__ = 'journal'
    id = db.Column(db.Integer, primary_key=True)
    time_stamp = db.Column(db.String(32), nullable=False)
    detail = db.Column(db.String(233), nullable=False)
    location = db.Column(db.String(80), nullable=False)

    def __repr__(self):
        return '<Journal %r>' % self.id

    def add(self):
        try:
            db.session.add(self)
            db.session.commit()
        except exc.IntegrityError:
            db.session.rollback()
            raise x_error.ServiceError('Failed to add journal record.')
        return self

    def modify(self):
        try:
            db.session.commit()
        except exc.IntegrityError:
            db.session.rollback()
            raise x_error.ServiceError('Failed to modify journal record.')
        return self

    def delete(self):
        try:
            db.session.delete(self)
        except exc.IntegrityError:
            db.session.rollback()
            raise x_error.ServiceError('Failed to delete journal record.')
