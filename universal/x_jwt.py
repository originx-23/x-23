...
def create_token(role, user_name, ip, day=15):
    ...
    payload = {
        ...
        'ip': ip,
        'user_name': user_name,
        'exp': datetime.datetime.utcnow() + datetime.timedelta(days=day)
        }
        ...
        def _check_token(*args, **kwargs):
            try:
                ...
                if data['ip'] != str(request.remote_addr):
                    if (data['ip'] == "127.0.0.1") and (str(request.remote_addr) != ipaddr):
                        raise x_error.Unauthorized("jwt check failed: error ip")
                # 非法的role
                if data['role'] not in role_map:
                    raise x_error.Unauthorized("jwt check failed: error role: %s" % data['role'])
                # 判断是否有权限
                if role_map.index(data['role']) < role_map.index(role):
                    raise x_error.Unauthorized("jwt check failed: role not allow")
